...
#always remember to change the dates to the Date format
...
ggplot(USGS.flow.data, aes(x = discharge.mean.approval)) +
  geom_bar()
# useful for categorical data, only one category
...
#binwidth: changes the binwidth lol
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10)

#bins: 20 bins, default is 30. generally do between 5-20 bins
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

#limits: zooming in on the chart and focusing on x= 0 to 500
ggplot(USGS.flow.data, aes(x = discharge.mean)) +
...
# not all skewed
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = gage.height.mean), bins = 20) +
...
#look at gage.height.mean, but color the values by mean approval (separates the A and P records)
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
#by having y = gage.height.mean, we are parsing our data into two categories: more informative.
# can take a look at the distribution, including the outliers
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean.approval, y = gage.height.mean))

#analogous to histograms: looking at the distribution along a slice of data.
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

#look at range and distribution within the range. shows the amount of data within the range
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
#the gage height doesnt increase as much as the discharge
ggplot(USGS.flow.data) +
