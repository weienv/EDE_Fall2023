...
## Set up
```{r, message = FALSE}
#ANOVA: if 3+ populations have equal means 
#ANOVA: analysis of variance
## factor: quantity under examination in an experiment as a possible cause of variation
#levels: populations/ categories, measurements, strata of a factor of interest in the current experiment
#assumptions: all pops are normally distributed
#pop variances are equal
#obseravtions are independent
#balance desgin: when all factors have equal sample sizes
library(tidyverse)
...
library(ggplot2)
library(dplyr)
Litter <- read.csv(here("Data/Processed_KEY/Processed_KEY/NEON_NIWO_Litter_mass_trap_Processed.csv"), stringsAsFactors = TRUE)
...
#check all value of plotID so that we know if each site has approximately the same number of things
...
#results: reject null in all except two: NIWO_057 and NIWO_046 because all others have a really small p value --> rejecting the null hypothesis --> the dataset doesnt follow a normal distribution
#but method is robust from small deviations from normal distribution
# doesnt follow the ANOVA test
...
#small p-value--> results: reject null i.e. variances are not equal

# Format ANOVA as aov
Litter.Totals.anova <- aov(data = Litter.Totals, dryMass ~ plotID)
summary(Litter.Totals.anova)
#### df: is 11, which means taht we have 11+1 levels
#### sum sq: deviation of each observation from the mean
#### mean sq: sum sq/df
#### f value: mean sq of plotID/mean sq of residuals
#### pr(>F) is the P value, really small --> reject null --> means are not all the same
...
###all std are with respect to the intercept
...
#### groups with the same letter e.g. a, abc. abcd all have a, which means they have the same mean
...
## would be better to rearrange the order of the sites so that a's are together and so on
```
...
bn ##use * to see the interaction among them
...
#litter.test <- cbind(Litter, Litter.interaction)
 ...
 #Litter.test <- cbind(Litter,Litter.interaction)
#compare means of each of the 24 levels, using one way anova as a result of 2 categorical variables
