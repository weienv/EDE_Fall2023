...
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.dat a, lakename == "Paul Lake" | lakename == "Peter Lake")
##excluding the ones that are not peter/paul
...
###filtering the range of dates from June 1st to Oct 31st
...
NTL.phys.data.JunethruOctober4 <- filter(NTL.phys.data, daynum %in% c(152:304)) #### rmb 152 and 304 will be included
...
### we need to use as.date
# Exercise: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.data.tuesday1990s <- filter(NTL.phys.data, year4 %in% c(1990,1999) & lakeid =="T")
...
```{r selecting columns}
##create new dataframe and then select certain columns
...
# note month = motn(sampledate) works cuz sampledate is a date object
...
##using select to arrange columns 
