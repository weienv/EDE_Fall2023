...
library(tidyverse);library(here);library(lubridate);library(shiny);library(rmarkdown)
...
plot_it <- function(the_parameter, start_year, end_year){
  ...
  ggplot(
    aes(
      x = sampledate, 
      y = !!sym(the_parameter), #we cant just put "the_parameter" here
      fill = depth_id, 
      shape = lakename)) +
  geom_point(alpha = 0.8, size = 2) +
  scale_x_date(
...
numericInput(inputId = "n", labe = "How many cars?", value = 5)
