...
#Read Eno river data
EnoDischarge <- read.csv("./Data/Processed_KEY/Processed_KEY/USGS_Site02085000_Flow_Processed.csv",
...
```{r}
?stl
###	*univariate* time series. Dont provide more than 2 variables!!! run stl for each variable
##s.window: periodic data. e.g. s.window = 365 (daily data)
...
### adding two more components to the dataframe
...
#be careful with the less than 0 values
...
###we need to replace the NAs instead of removing them in TSA
...
* **Augmented Dickey Fuller**: no seasonality, non-parametric, missing data not allowed. Function: `adf.test()` (package: tseries) ###important for forecasting. for stocastic trend, not monotonic
...
f_year <- 1984 ###defining the first year and first month
...
##p-value = 1.822e-11 --> too small --> reject null hypothesis --> there is a trend
