...
```{r Read in road data}
#Read a spatial dataset off the web
...
class(GasPlants)
names(GasPlants)
str(GasPlants$geometry) #sfc: simple feature column
...
EPAair <- read.csv(here("Data/Processed_KEY/Processed_KEY/EPAair_O3_PM25_NC1819_Processed.csv"), stringsAsFactors = TRUE) %>% 
...
EPAair.sf <- EPAair.sites %>% 
  st_as_sf(coords = c('mean_Lng','mean_Lat'), #long is x and lat is y
           crs=4326)
class(EPAair.sf)
...
#Upgrade to a spatial dataframe, add attributes # sf = spatial dataframe
...
the_buffer <- sf_GraingerHall %>% st_buffer(1) #one degree of long and lat. not spherical because our computers are planes but the coordinates are spherical
...
Look for the different numbers here: epsg.io
...
EPAair.sf.utm <- st_transform(EPAair.sf,32617) #reprojecting the data to 32617
...
ggplot() +
  ...
  geom_sf(data = GasPlants.utm,color='red',shape='O') #lines are bent:: projected

#Create a distance matrix between the point in each dataset
distance_matrix <- st_distance(EPAair.sf.utm, GasPlants.utm, pairwise=T) #pairwise: matrix
