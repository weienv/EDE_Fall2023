...
NTL.nutrient.data.PeterPaul <- droplevels(NTL.nutrient.data.PeterPaul) ###for dropping all the other lakenames
summary(NTL.nutrient.data.PeterPaul$lakename)
summary(NTL.nutrient.data.PeterPaul$comments) #repeating null for 2770 times
...
NTL.nutrient.data.PeterPaul <- 
  NTL.nutrient.data.PeterPaul %>% #beginning
  mutate(month = month(sampledate)) %>% # creating a new month column
  select(lakeid:daynum, month, sampledate:comments) %>% # moving the month column to the right of daynum
  drop_na(depth)
...
NTL.nutrient.data.PeterPaul.subset <- select(NTL.nutrient.data.PeterPaul, 
                                             lakename:daynum, month, sampledate, depth:po4)
# we could also just remove the ones that we dont want
...
#the dataset now has more rows. the code below combines the differnt nutrient columns into two, one with the nutrient name and one with the values
...
# transforming the longer version of of the dataset to the original one (separting the nutrient columns)
...
Litter <- LitterFiles %>%
  plyr::ldply(read.csv) #row binding all the files in the folder niwo_litter
```
...
LitterTrap <- left_join(Litter, Trap, by = c("siteID", "plotID", "trapID")) #same number of observation as litter and adding the columns from trap
...
NTL.PeterPaul.summaries <- 
  ...
  filter(!is.na(temperature_C) & !is.na(tn_ug) & !is.na(tp_ug)) %>% ### none of the three is na 
