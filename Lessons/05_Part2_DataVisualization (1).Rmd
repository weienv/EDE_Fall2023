...
O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_bw() #####black on white (background) with black blackground lines
print(O3plot2)
...
theme_set(mytheme) ###setting the default theme to be a certain theme
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) + #horizontal line, lty type of the line
  ...
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")),  #plot only a section of the dates
    date_breaks = "2 months", date_labels = "%b %y") +
  ylab(expression("O"[3]* " AQI Value")) + ##changing the y label, "expression()"is not needed if theres no subscript
  theme(axis.text.x = element_text(angle = 45,  hjust = 1))
print(O3plot7)  
#hjust: horizontal justification, 1: left aligning the text
```
...
NvsP2 <-
  ...
  scale_shape_manual(values = c(15, 17)) + #shape of the dots are c hanged
...
##plotting two plots at the same time. rel_heights adjusts teh height of each plot
```
