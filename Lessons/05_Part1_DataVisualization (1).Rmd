...
```{r, fig.height = 3, fig.width = 4, warning = FALSE, fig.cap= "figure 1"}
###absolute 
# Scatterplot
...
O3plot2 <- ggplot(EPAair) +
  ##geom_point(aes(x = Date, y = Ozone, color = "blue")) ###when we do this, we are adding another layer to the plot
...
PMplot.faceted <-
  ...
  geom_point() +
  facet_wrap(vars(Site.Name), nrow = 3) ###facet_wrap: creating different plots for different sites, 3 rows
...
# Scatterplot
lightvsDO <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = irradianceWater, y = dissolvedOxygen)) + ##visualizing 2 vairables n finding relationship between them
  geom_point()
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? ##fill is the filling of the boxplot, whereas color changes the lines of the boxplots and the outlier dots
print(Nutrientplot3)
...
#### position: dodge: so that the dots do not overlay, they go to the side (hence horizontal "lines" in some plots)
