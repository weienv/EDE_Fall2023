...
epa_pm25_sites_sf <- read_csv(here('Data/Raw/EPAair_PM25_NC2018_raw.csv')) %>% 
  ...
  st_as_sf(coords = c('SITE_LONGITUDE','SITE_LATITUDE'), crs=4269) ###converting into spatial dataframe
...
#geoid: fips code, unique for each county
...
#Reveal the spatial extent of this dataset via the st_bbox() function
st_bbox(counties_sf)
...
NCHUC<- st_read(here('Data/Spatial/NCHUC8.shp')) 
#Reveal the columns
colnames(NCHUC)
...
#Create a map
mapview(NCHUC, col.regions = 'orange') + 
  mapview(UN_sf, col.regions = 'purple')
...
svi2018_nc_raw <- read.csv(
  ...
  colClasses = c('FIPS' = 'factor')) %>% ###fips is not a number, we need the preceeding 0s 
...
#### the GEOID in the counties_sf is the FIPS in svi2018 file
...
mapview(counties_sf_join, 
        zcol = 'E_POV', ###color by ppl in poverty
        col.regions = brewer.pal(2, 'RdBu')) +  ##specify color ramp
  mapview(epa_pm25_sites_sf, cex = 'maxPM') ###add another map: maximum particulate matters
...
wind_raw <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv',
  ...
  mutate(NCHUC = substr(HUC_12,1,8)) %>% ###grabbign first to 8th value
  group_by(NCHUC) %>% 
  summarize(SumWindEnergy = sum(AvgWindEnergy))
  
#Join to HUC_8 features
huc8_sf_join = merge(x = NCHUC,
                     y = wind_raw,
                     by.x = 'HUC_8',
                     by.y = 'NCHUC')
...
mapview(huc8_sf_join, zcol = 'SumWindEnergy')
```
...
state_sf <- counties_sf %>% 
  group_by('STATEFP') %>% #group all counties into one
...
list(NCHUC$DWQ_Basin)

#Summarize on DWQ Basin value
NCHUC2 <- NCHUC %>% 
  group_by('DWQ_Basin')%>% #group all counties into one
  summarize(ALAND = sum(SQ_MILES))%>%
  summarize(ACRES = sum(ACRES))

#Map the data

mapview(NCHUC2, zcol = 'ACRES')
```

## 3.2 Transforming coordinate reference systems of datasets m
...
huc8_sf_utm <-     st_transform(NCHUC, crs = 26917)
huc2_utm <-        st_transform(NCHUC2, crs = 26917)
```
...
neuse_intersect_1 <- counties_sf_utm[neuse_sf,] ###selects everything that intersects with neuse_sf

#Select intersecting counties using the `filter()` command
neuse_intersect_2 <- counties_sf_utm %>% 
  filter(st_intersects(x = ., y = neuse_sf, sparse = FALSE)) ##### x= . is what i put into the filter command. y is the feature to select the counties

#View the result
myMap + mapview(neuse_intersect_1, alpha.regions = 0) #spatial overlay

#Actually intersect the counties (not just select those that intersect)
neuse_counties_sf <- neuse_sf %>%  ###neuse_sf is the cookie cutter, and counties_sf_utm is the cookie dough
  st_intersection(counties_sf_utm)
myMap + mapview(neuse_counties_sf, alpha.regions = 0)
#the aland and awater is not updated
...
site_30km <- theSite_sfc_transformed %>% 
...
#Right order, pay attention to the order we add stuff in
ggplot() +
  geom_sf(data = state_sf_utm, color='red',size=2) +
  geom_sf(data = counties_sf_utm, aes(fill = ALAND), color = 'white')  +
...
huc8s_WGS84 <- st_transform(NCHUC,c=4326)
huc2_WGS84<- st_transform(NCHUC2,c=4326)

#Import the leaflet library
library(leaflet)

#Now plot with leaflet: no errors as all layers are in Leaflet's native CRS (WGS84) ###when using leaflet, do not use a projected coordinate system
leaflet() %>% addTiles() %>% 
