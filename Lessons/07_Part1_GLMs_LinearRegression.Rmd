...
PeterPaul.chem.nutrients <- read.csv(here("Data/Processed_KEY/Processed_KEY/NTL-LTER_Lake_Chemistry_Nutrients_PeterPaul_Processed.csv"), stringsAsFactors = TRUE)
...
#we want standard error to be as small as possible
#r^2: 0.3 --> depth explains 30% of variation in radience
#the small p value provides us confidence that we have done a meaningful regression
...
#srong negative corelation: -0.5559
#we need to then check residuals
```
...
#normal q-q: the dots should follow the dashed line. lower and higher ends could deviate from the dashed line (normal). the one here is not normal for a normal qq plot
#scale-location: should look for symmytry with the red line
#residuals vs. leverage: look for potential outliers and dots that deviate a lot from the red line
par(mfrow = c(1,1))
```
...
#clearly see no linear relationship between x and y
...
PeterPaul.chem.nutrients <- filter(PeterPaul.chem.nutrients, 
                                   irradianceWater != 0 & irradianceWater < 5000) ###removes 0 for log transformation, and removes the outlier
...
par(mfrow = c(1,1))
#with log transformation, the r squared value has gone up
# Add a line and standard error for the linear regression
#residuals plot looks a lot better than before
irradiancebydepth2 <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
...
irradiancebydepth2 <- 
    ...
    geom_smooth(method = 'lm', se = FALSE, color = "black") ####se = false: eliminate confidence intervals 
print(irradiancebydepth2)
...
plot(TPregression)
par(mfrow = c(1,1))
#residuals vs. fitter: the red line should be FLAT
...
PeterPaulnutrients <- 
  ...
  select(tn_ug:po4) %>%
  na.omit()
# a way to find variables that are highly correlated
PeterPaulCorr <- cor(PeterPaulnutrients)
corrplot(PeterPaulCorr, method = "ellipse")
##thinner the plot, the higher the correlation
corrplot.mixed(PeterPaulCorr, upper = "ellipse")
#this plot shows the value of the correlation
```
...
step(TPAIC) ###shows the AIC number when one variable is removed
#this formuala also spits out the best fit model
